...
EXPIRY_DATE = datetime(2025, 1, 15)  # Set expiry date to 1st November 2024
...
def load_approved_users():
    if os.path.exists(APPROVED_USERS_FILE):
        with open(APPROVED_USERS_FILE, "r") as file:
            ...
            for line in file:
                stripped = line.strip()
                if stripped.isdigit():  # Check if the line is a valid number
                    approved_users.add(int(stripped))
                elif stripped:  # Log or handle non-empty invalid lines
